...
describe('logoutFactory', () => {
  ...
  test('should set an empty cookie with expiration time', () => {
    ...
    expect(res.cookie).toHaveBeenCalledWith(`${ACCESS_TOKEN_COOKIE_NAME}-${did}`, '', { ...COOKIES_ATTRIBUTES, expires: expectedCookieExpiration })
    expect(res.cookie).toHaveBeenCalledWith(`${REFRESH_TOKEN_COOKIE_NAME}-${did}`, '', { ...COOKIES_ATTRIBUTES, expires: expectedCookieExpiration })
  })
})
